...
function SessionList({ onSelect }: { onSelect: (s: SessionMeta) => void }) {
  ...
  // 방 이름별 그룹화
  const grouped = sessions.reduce<Record<string, SessionMeta[]>>((acc, s) => {
    (acc[s.room] ??= []).push(s);
    return acc;
  }, {});
      ...
      {Object.entries(grouped).map(([room, list]) => (
        <div key={room}>
          ...
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-muted-foreground text-xs">{list.length}개 세션</span>
          </div>
...
function ConversationView({ session, onBack }: { session: SessionMeta; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    es.onmessage = (e) => {
      try { setLog(JSON.parse(e.data)); } catch { /* ignore */ }
    };
    return () => es.close();
  }, [session.filename]);
              ...
              log.entries.map((entry, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span className={`mb-1 block text-xs font-semibold ${isAgent ? 'text-left' : 'text-right'}`}>
                        {speakerName}
                      </span>
